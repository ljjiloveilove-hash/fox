(async function () {
  ...
  function cleanup() { log('🧹 开始退出清理'); Object.entries(state._timers).forEach(([key, timer]) => { if (timer !== null) { clearTimeout(timer); state._timers[key] = null; } }); unbindEvents();['ss-float-bubble', 'ss-fox-styles', 'ss-mobile-panel-fix', 'ss-fox-ui-polish', 'ss-dashboard', 'ss-dash-overlay', 'ss-hushen-panel', 'ss-hushen-overlay', 'ss-husou-panel', 'ss-husou-overlay', 'ss-beat-memory-panel', 'ss-beat-memory-overlay', 'ss-custom-edit-panel', 'ss-custom-edit-overlay'].forEach(id => { const el = topDoc.getElementById(id); if (el) el.remove(); }); try { localStorage.removeItem(CONFIG.PANEL_KEY); } catch (e) { } disableAllBeauty(); disableAllMini(); setThoughtVisible(true); log('🧹 退出清理完成'); }
  ...
  const getPreset = (...args) => getFn('getPreset')?.(...args);
  const updatePresetWith = (...args) => getFn('updatePresetWith')?.(...args);
  const eventOn = (...args) => getFn('eventOn')?.(...args);
  const getPresetNames = (...args) => getFn('getPresetNames')?.(...args);
  const replacePreset = (...args) => getFn('replacePreset')?.(...args);
  const setPreset = (...args) => getFn('setPreset')?.(...args);
  const tavern_events = getFn('tavern_events') || {
  ...
  function getActiveRulesSummary() { try { const prompts = getPreset('in_use').prompts; const lines = []; let totalLength = 0; const maxTotalLength = 4000; const skipIds = ['a00fb1fe-9907-4709-b230-43ae0d236529', '7a3c853f-88ff-48ff-8c6a-020d9bbeb3d8', 'jailbreak', '2f2729f4-5d93-4268-a242-165a9c894e60', '1f2c1af6-cd5d-4416-b990-cf84d67c4854', 'agentSystemPrompt', '8b7ac605-f910-4ad9-9dd8-51004b464fab', '6589590f-c84c-4599-9540-94e14692ac9d', 'agentResults']; const skipNames = ['初始化(不要关)', '(别关)注入强调要求', '--开始--', '阅读（不要关掉！）', '大总结填入', 'Agent System Prompt', '--直出输出(think尾部)', '大总结输出(不要关)', 'Agent Results']; const skipIdentifiers = ['a00fb1fe-9907-4709-b230-43ae0d236529', '7a3c853f-88ff-48ff-8c6a-020d9bbeb3d8', 'jailbreak', '2f2729f4-5d93-4268-a242-165a9c894e60', '1f2c1af6-cd5d-4416-b990-cf84d67c4854', 'agentSystemPrompt', '8b7ac605-f910-4ad9-9dd8-51004b464fab', '6589590f-c84c-4599-9540-94e14692ac9d', 'agentResults']; for (const p of prompts) { if (totalLength >= maxTotalLength) break; if (!p?.enabled || !p?.content?.trim() || p.system_prompt) continue; if (skipIds.includes(p.id)) continue; if (skipNames.includes(p.name)) continue; if (skipIdentifiers.includes(p.identifier)) continue; const name = p.name || p.identifier || p.id; const setvarMatch = p.content.match(/\{\{setvar::\w+::([\s\S]*?)\}\}/); let raw = setvarMatch ? setvarMatch[1].trim() : p.content; raw = raw.replace(/\{\{\/\/[\s\S]*?\}\}/g, ''); raw = raw.replace(/\{\{user\}\}/g, '用户'); raw = raw.replace(/\{\{getvar::\w+\}\}/g, ''); raw = raw.replace(/\{\{lastUserMessage\}\}/g, ''); raw = raw.replace(/\{\{trim\}\}/g, ''); raw = raw.replace(/\{\{[^}]*\}\}/g, ''); const paragraphs = raw.split('\n\n'); let brief = ''; for (const para of paragraphs) { const t = para.trim(); if (!t) continue; if ((brief + '\n\n' + t).length > 500) break; brief = brief ? brief + '\n\n' + t : t; } if (brief.trim()) { const line = `【${name}】\n${brief}`; if (totalLength + line.length > maxTotalLength) break; lines.push(line); totalLength += line.length + 2; } } return lines.join('\n\n'); } catch (e) { return ''; } }
  ...
  async function runFullSummary() {
    if (state.isSummarizing) return; const glmi = getFn('getLastMessageId'), gcm = getFn('getChatMessages'), scm = getFn('setChatMessages'), gr = getFn('generateRaw'); if (!glmi || !gcm || !scm || !gr) return; const lastId = glmi(); if (lastId < 0) return; state.isSummarizing = true; const btn = topDoc.getElementById('ss-dash-summary'); if (btn) { btn.textContent = '⏳ 总结中...'; btn.disabled = true; } let persistToast = null; try { const t = topWindow?.toastr || window.toastr; if (t?.info) { t.info('🦊 大总结进行中，请稍候……', '🦊玄狐', { timeOut: 0, extendedTimeOut: 0 }); persistToast = topDoc.querySelector('.toast-info') || topDoc.querySelector('#toast-container .toast-info'); } } catch { } try {
      const messages = gcm(`0-${lastId}`) || [];
      const ctx = messages.map(m => `${m.role === 'assistant' ? 'AI' : '用户'}：${m.message || ''}`).join('\n\n');
      const es = getPromptContentById(CONFIG.SUMMARY_ID);
      const ep = es ? `\n\n【上次总结内容，请在此基础上补充新内容】\n${es}` : '';
      const sp = '\n\n请根据以上聊天记录整理一份可供后续续写使用的全文总结。保留关键人物、关系、设定、事件进展、未解决线索与重要约定，避免无关细节。';
      const r = await gr({ user_input: ctx + ep + sp, should_silence: true, should_stream: false, max_chat_history: 0, ordered_prompts: ['user_input'] }); const txt = typeof r === 'string' ? r : r?.content || ''; if (txt.trim()) { await setPromptContentById(CONFIG.SUMMARY_ID, txt.trim()); toast('✅ 全文大总结已完成并注入预设', 'success'); } else { toast('⚠️ 总结返回空内容', 'warning'); }
    } catch (e) { log('大总结失败', e); toast('❌ 总结失败', 'error'); } finally { try { if (persistToast?.remove) persistToast.remove(); } catch { } try { const t = topWindow?.toastr || window.toastr; if (t?.clear) t.clear(); } catch { } state.isSummarizing = false; if (btn) { btn.textContent = '📝 全文大总结'; btn.disabled = false; } }
  }
  ...
  function openCustomEditPanel(entryId, entryName, knownVar = '') {
    ...
    overlay.id = 'ss-custom-edit-overlay';
  ...
  function loadHushenSettings() { try { const raw = localStorage.getItem(CONFIG.HUSHEN_STORAGE_KEY); return raw ? { firstRun: false, analyzed: {}, drafts: {}, minWords: 100, maxWords: 250, style: 'detailed', ...JSON.parse(raw) } : { firstRun: true, analyzed: {}, drafts: {}, minWords: 100, maxWords: 250, style: 'detailed' }; } catch { return { firstRun: true, analyzed: {}, drafts: {}, minWords: 100, maxWords: 250, style: 'detailed' }; } }
  ...
  async function husouSearchJina(keyword, signal, s) {
    try {
      const maxLen = s.maxExtractLength || 40000;
      const resp = await fetch(`https://s.jina.ai/${encodeURIComponent(keyword)}`, {
        signal,
      ...
      if (!text) return null;
      const clean = text.replace(/\s+/g, ' ').slice(0, maxLen).trim();
      return clean ? `【Jina搜索：${keyword}】${clean}` : null;
    } catch (e) {
      if (e.name !== 'AbortError') log('Jina搜索失败:', keyword, e);
      return null;
    }
  }
  async function husouSearchAll(keywords, s) {
    ...
    try {
      ...
      for (const kw of keywords) {
        ...
        if (s.enableJinaFetch) tasks.push({ kw, promise: husouSearchJina(kw, ctrl.signal, s) });
      }
  ...
  async function husouManualSearch(messageId) {
    ...
    if (!s.enableWikipedia && !s.enableMoegirl && !s.enableCustomSearch && !s.enableJinaFetch) { toast('请至少启用一个搜索来源', 'warning'); return; }
...
<div id="search_engine_status" style="color:#706058;font-size:10px;margin-top:4px;">当前状态：🦊狐搜 已加载</div></div>`;
  ...
  function bindEvents() { unbindEvents(); try { if (typeof eventOn !== 'function') return; const h1 = tavern_events?.GENERATION_STARTED ? eventOn(tavern_events.GENERATION_STARTED, onGenStart) : null; const h2 = tavern_events?.GENERATION_ENDED ? eventOn(tavern_events.GENERATION_ENDED, onGenEnd) : null; if (h1) state.eventHandles.push(h1); if (h2) state.eventHandles.push(h2); } catch (e) { log('绑定事件失败', e); } }
  ...
  function createLeftRightSwitch(lLabel, rLabel, lId, rId, onChange) { const w = topDoc.createElement('span'); w.style.cssText = 'display:inline-flex;gap:0;border-radius:6px;overflow:hidden;border:1px solid rgba(232,120,74,0.25);flex-shrink:0;'; const lB = topDoc.createElement('span'); lB.textContent = lLabel; const rB = topDoc.createElement('span'); rB.textContent = rLabel;[lB, rB].forEach(b => { b.style.cssText = 'padding:4px 12px;cursor:pointer;font-size:11px;font-weight:600;transition:all 0.15s;white-space:nowrap;'; }); function paint(btn, active) { btn.style.background = active ? 'rgba(232,120,74,0.25)' : 'transparent'; btn.style.color = active ? '#f0a070' : '#555'; } function up() { const lOn = isPromptEnabledInPreset(lId); const rOn = isPromptEnabledInPreset(rId); paint(lB, lOn); paint(rB, rOn); } lB.onclick = async () => { await togglePromptById(lId, true); await togglePromptById(rId, false); up(); if (onChange) onChange('left'); }; rB.onclick = async () => { await togglePromptById(lId, false); await togglePromptById(rId, true); up(); if (onChange) onChange('right'); }; w.appendChild(lB); w.appendChild(rB); up(); return { wrapper: w, update: up }; }
...
`;
  ...
  async function initUI() {
    ...
    bubble.addEventListener('pointercancel', () => { dragging = false; dragMoved = false; bubble.style.transform = ''; bubble.style.pointerEvents = ''; bubble.style.transition = ''; document.body.focus(); });
...
@media (max-width: 760px) {
  ...
  function enhanceDashboardUI() {
    ...
    for (const p of pairs) {
      ...
      if (p.box) dash.appendChild(p.box);
    }
  ...
  async function init() {
    ...
    const initTimer = setTimeout(async () => {
      ...
      updateBubbleUI();
      log('🎉 完成|', buildStatusText(), '|流式:', isStreamingEnabled());
    }, CONFIG.INIT_DELAY);
